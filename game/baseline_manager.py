...
class BaselineManager:
        ...
        for entity in entities:
            if "StudentDesk" in entity.id:
                data = {
                    ...
                    "desk_state": copy.deepcopy(
                        entity.get_component("StudentDeskState")
                    ),
                    "anim_state": copy.deepcopy(
                        entity.get_component("AnimState")
                    ),  # Might be None
                }
        ...
        for entity in entities:
            if entity.id in self.snapshot:
                ...
                entity.add_component(
                    "StudentDeskState", copy.deepcopy(snap["desk_state"])
                )
    ...
    def is_desk_anomalous(self, entity):
        ...
        if (snap["anim_state"] is None) != (current_anim is None):
            print(
                f"[Trace] {entity.id}: Occupancy mismatch. Baseline: {snap['anim_state'] is not None}, Current: {current_anim is not None}"
            )
            return True
        ...
        if snap["anim_state"] and current_anim:
            ...
            if s.name != c.name:
                print(
                    f"[Trace] {entity.id}: Name mismatch. Baseline: {s.name}, Current: {c.name}"
                )
                return True
            if s.id_number != c.id_number:
                print(
                    f"[Trace] {entity.id}: ID mismatch. Baseline: {s.id_number}, Current: {c.id_number}"
                )
                return True
            if s.cloth_color != c.cloth_color:
                print(
                    f"[Trace] {entity.id}: Cloth mismatch. Baseline: {s.cloth_color}, Current: {c.cloth_color}"
                )
                return True
            if s.pen_color != c.pen_color:
                print(
                    f"[Trace] {entity.id}: Pen mismatch. Baseline: {s.pen_color}, Current: {c.pen_color}"
                )
                return True
            if s.is_alien != c.is_alien:
                print(
                    f"[Trace] {entity.id}: Alien flag mismatch. Baseline: {s.is_alien}, Current: {c.is_alien}"
                )
                return True
            if s.is_dancing != c.is_dancing:
                print(
                    f"[Trace] {entity.id}: Dancing flag mismatch. Baseline: {s.is_dancing}, Current: {c.is_dancing}"
                )
                return True
            if s.is_ghost != c.is_ghost:
                print(
                    f"[Trace] {entity.id}: Ghost flag mismatch. Baseline: {s.is_ghost}, Current: {c.is_ghost}"
                )
                return True
        ...
        if snap["desk_state"] and current_desk:
            ...
            if s_d.calculator.is_visible != c_d.calculator.is_visible:
                print(f"[Trace] {entity.id}: Calc visibility mismatch.")
                return True
            if s_d.smartphone.is_visible != c_d.smartphone.is_visible:
                print(f"[Trace] {entity.id}: Phone visibility mismatch.")
                return True
            if s_d.cheatsheet.is_visible != c_d.cheatsheet.is_visible:
                print(f"[Trace] {entity.id}: Cheatsheet visibility mismatch.")
                return True
            if s_d.exam_sheet.extra_logs != c_d.exam_sheet.extra_logs:
                print(f"[Trace] {entity.id}: Exam sheet logs mismatch.")
                return True
