...
from components.cheatsheet_state import CheatsheetState
...
def display():
    ...
    cheatsheet_state.update(dt)

    # Only allow movement if not actively inspecting the object
    if not cheatsheet_state.is_being_inspected:
        yr = math.radians(cam_yaw)
        fwd_x = math.sin(yr)
        fwd_y = math.cos(yr)
        rgt_x = math.cos(yr)
    ...
    draw_floor()
    draw_desk()
    renderer.render(desk_transform, cheatsheet_state)

    if not cheatsheet_state.is_being_inspected and dist_to_desk() < INTERACT_RANGE:
        draw_prompt("[E] Inspect Paper")
...
def keyboard_down(key, x, y):
    ...
    if key == b"e":
        if not cheatsheet_state.is_being_inspected and dist_to_desk() < INTERACT_RANGE:
            cheatsheet_state.inspect()
            print("[Game] Inspecting Cheat Sheet.")
    elif key == b" ":
        if cheatsheet_state.is_being_inspected:
            cheatsheet_state.release()
            print("[Game] Cheat Sheet returned to desk.")
    elif key == b"\x1b":
        sys.exit()
...
def main():
    ...
    glutCreateWindow(b"Cheat Sheet Interaction Test")
    glEnable(GL_DEPTH_TEST)
    ...
    print("--- Cheat Sheet Test Controls ---")
    print("W/A/S/D    : Move Camera")
    print("Arrow Keys : Look")
    print("E          : Inspect paper (when close)")
